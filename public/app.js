...
(() => {
  ...
  function openOrderViewer(initialOrders, startIndex = 0, opts = {}) {
    ...
    // 바코드 방향: 'horizontal' | 'vertical' — 저장된 선호값 로드 (기본 가로)
    let barcodeOrient = localStorage.getItem('barcodeOrient') === 'vertical' ? 'vertical' : 'horizontal';
    ...
    function renderCard(animDir = 0) {
      ...
      // 바코드(조식)일 때만 방향 전환 버튼 노출
      const showOrientToggle = !isLateNight;
      const isVertical = barcodeOrient === 'vertical';
      ...
      card.innerHTML = `
        ...
        ` : `
          ${showOrientToggle ? `
          <div class="orient-toggle-row">
            <button class="orient-toggle ${!isVertical ? 'active' : ''}" data-orient="horizontal">─ 가로</button>
            <button class="orient-toggle ${isVertical ? 'active' : ''}" data-orient="vertical">│ 세로</button>
          </div>
          ` : ''}
          <div class="barcode-wrap ${isVertical ? 'vertical' : ''}">
      ...
      if (!isLateNight) {
        ...
        // 방향 전환 버튼 이벤트
        card.querySelectorAll('[data-orient]').forEach(b =>
          b.addEventListener('click', () => {
            barcodeOrient = b.dataset.orient;
            localStorage.setItem('barcodeOrient', barcodeOrient);
            renderCard(0);
          }));
      }
